...
본 문서는 AXI-Lite IP에서 하드웨어(FPGA 로직)에 의해 지속적으로 업데이트되고, 프로세서(MicroBlaze)에게는 읽기 전용으로 제공되는 레지스터(`slv_reg0`)를 구현하는 방법에 대해 상세히 설명합니다.

## 1. 사용자 제안 분석: `slv_reg0` 업데이트 방식에 대한 질문

사용자께서는 `myip_ultrasonic_fsm_v1_0_S00_AXI.v` 파일의 AXI Write 로직(`always @(posedge S_AXI_ACLK)` 블록) 내 `if (slv_reg_wren)` 문의 `else` 브랜치에 `slv_reg0[13:0] <= digit_for_display;`와 같이 값을 할당하는 방식을 제안하셨습니다.
    ...
    if (slv_reg_wren) // AXI Write가 있을 때
      begin
        ...
        endcase
         ...
         slv_reg0[13:0] <= digit_for_display; // <--- 사용자 제안 부분
    end
  end
end
```
...
2.  **읽기 전용 레지스터의 목적**:
    -   우리의 목표는 `slv_reg0`를 프로세서가 거리 값을 **읽어가는(Read-Only)** 레지스터로 사용하는 것입니다. 프로세서가 이 레지스터에 값을 **쓰는(Write)** 것은 원치 않습니다. 제안 방식은 `slv_reg0`를 AXI Write 로직에 의해 쓰여질 수도 있고, 하드웨어에 의해 쓰여질 수도 있는 복잡한 레지스터로 만듭니다.
...
### 2.1 `slv_reg0`를 AXI Write 로직에서 제거 (프로세서가 쓰지 못하게 함)
...
이제 `slv_reg0` 변수 자체가 `distance_from_fsm` 값을 가지고 있으므로, AXI Read 로직은 `slv_reg0`를 직접 읽도록 되돌립니다.
...
이러한 수정들을 통해 `slv_reg0`는 하드웨어(`distance_from_fsm`)에 의해 지속적으로 업데이트되고, 프로세서는 이 값을 읽기만 할 수 있는 견고한 읽기 전용 레지스터로 기능하게 됩니다. 이 방식이 더 '정석적'이며, `slv_reg0`의 역할이 명확해집니다.
...
*   **하드웨어에 의한 쓰기 (Write by Hardware):**
    ...
    *   프로세서(MicroBlaze)는 `slv_reg0`에 직접 값을 **쓸 수 없습니다.** AXI Write 로직에서 `slv_reg0`에 대한 쓰기 케이스(`2'h0`)가 제거되었기 때문입니다.
