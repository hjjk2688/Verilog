...
xil_printf를 사용해야 할 때: (대부분의 경우 추천)
* 간단한 디버깅 메시지나 변수 값을 출력할 때
* 부동 소수점(float, double) 값을 출력할 필요가 없을 때
* 프로그램의 코드 크기나 메모리 사용량에 민감할 때
* 빠른 실행 속도가 필요할 때
> 결론: 일반적인 임베디드 환경에서는 리소스 절약을 위해 `xil_printf`를 사용하는 것이 기본입니다.

printf를 사용해야 할 때:
* 반드시 부동 소수점(`%f`) 값을 형식에 맞춰 출력해야 할 때
* 표준 C 라이브러리의 모든 서식 지정 기능이 필요할 때
* 코드 크기나 메모리 사용량에 큰 제약이 없을 때
